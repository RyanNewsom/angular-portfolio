import { InMemoryDbService } from 'angular-in-memory-web-api';
export class InMemoryDataService implements InMemoryDbService {
  createDb() {
    let blogPosts = [
      {
        id: 1,
        name: 'First Post',
        description: 'Built an Angular app today... it was great!',
        body: 'Strong, aged beans rich, aromatic, shop, cultivar, beans bar est robusta froth. Body, as, roast grounds, cup roast extra lungo id cream. Strong, decaffeinated brewed, percolator saucer, to go cortado qui cultivar roast. A, viennese coffee, instant turkish flavour steamed and espresso. \n Robusta iced milk chicory cup filter chicory wings. Roast chicory acerbic, wings latte, cup, macchiato, saucer bar cinnamon coffee single origin. Et blue mountain, breve aromatic mug cup breve. Decaffeinated, plunger pot breve, sugar, filter, arabica pumpkin spice affogato chicory milk latte.'
      },
      {
        id: 2,
        name: 'Second Post',
        description: 'Lorem Khaled Ipsum is a major key to success. You smart, you loyal, you a genius.',
        body: 'Strong, aged beans rich, aromatic, shop, cultivar, beans bar est robusta froth. Body, as, roast grounds, cup roast extra lungo id cream. Strong, decaffeinated brewed, percolator saucer, to go cortado qui cultivar roast. A, viennese coffee, instant turkish flavour steamed and espresso. \n Robusta iced milk chicory cup filter chicory wings. Roast chicory acerbic, wings latte, cup, macchiato, saucer bar cinnamon coffee single origin. Et blue mountain, breve aromatic mug cup breve. Decaffeinated, plunger pot breve, sugar, filter, arabica pumpkin spice affogato chicory milk latte.'
      },
      {
        id: 3,
        name: 'Third Post',
        description: 'How’s business? Boomin. I’m up to something. The key is to drink coconut, fresh coconut, trust me.',
        body: 'Strong, aged beans rich, aromatic, shop, cultivar, beans bar est robusta froth. Body, as, roast grounds, cup roast extra lungo id cream. Strong, decaffeinated brewed, percolator saucer, to go cortado qui cultivar roast. A, viennese coffee, instant turkish flavour steamed and espresso. \n Robusta iced milk chicory cup filter chicory wings. Roast chicory acerbic, wings latte, cup, macchiato, saucer bar cinnamon coffee single origin. Et blue mountain, breve aromatic mug cup breve. Decaffeinated, plunger pot breve, sugar, filter, arabica pumpkin spice affogato chicory milk latte.'
      },
      {
        id: 4,
        name: 'Fourth Post',
        description: '. The other day the grass was brown, now it’s green because I ain’t give up. Never surrender.',
        body: 'Strong, aged beans rich, aromatic, shop, cultivar, beans bar est robusta froth. Body, as, roast grounds, cup roast extra lungo id cream. Strong, decaffeinated brewed, percolator saucer, to go cortado qui cultivar roast. A, viennese coffee, instant turkish flavour steamed and espresso. \n Robusta iced milk chicory cup filter chicory wings. Roast chicory acerbic, wings latte, cup, macchiato, saucer bar cinnamon coffee single origin. Et blue mountain, breve aromatic mug cup breve. Decaffeinated, plunger pot breve, sugar, filter, arabica pumpkin spice affogato chicory milk latte.'
      },
      {
        id: 5,
        name: 'Fifth Post',
        description: 'You see that bamboo behind me though, you see that bamboo? Ain’t nothin’ like bamboo. Bless up.',
        body: 'Strong, aged beans rich, aromatic, shop, cultivar, beans bar est robusta froth. Body, as, roast grounds, cup roast extra lungo id cream. Strong, decaffeinated brewed, percolator saucer, to go cortado qui cultivar roast. A, viennese coffee, instant turkish flavour steamed and espresso. \n Robusta iced milk chicory cup filter chicory wings. Roast chicory acerbic, wings latte, cup, macchiato, saucer bar cinnamon coffee single origin. Et blue mountain, breve aromatic mug cup breve. Decaffeinated, plunger pot breve, sugar, filter, arabica pumpkin spice affogato chicory milk latte.'
      },
      {
        id: 6,
        name: 'Sixth Post',
        description: 'Put it this way, it took me twenty five years to get these plants, twenty five years of blood sweat and tears, and I’m never giving up, I’m just getting started.',
        body: 'Strong, aged beans rich, aromatic, shop, cultivar, beans bar est robusta froth. Body, as, roast grounds, cup roast extra lungo id cream. Strong, decaffeinated brewed, percolator saucer, to go cortado qui cultivar roast. A, viennese coffee, instant turkish flavour steamed and espresso. \n Robusta iced milk chicory cup filter chicory wings. Roast chicory acerbic, wings latte, cup, macchiato, saucer bar cinnamon coffee single origin. Et blue mountain, breve aromatic mug cup breve. Decaffeinated, plunger pot breve, sugar, filter, arabica pumpkin spice affogato chicory milk latte.'
      }
    ];

    return {blogPosts};
  }
}
